...
public class FillHelper {
	...
	{
		Random rand = new Random();
		int deathYear = birthYearStart + this.LIFE_SPAN;
		
		int firstJob = this.FIRST_JOB + birthYearStart;
		int retired = this.RETIREMENT_YEAR + birthYearStart;
		
		makeEvent(person, "birth", birthYearStart, name);
	...
	private void marry(Person father, Person mother, String name)
	{
		try {
			...
			if(events != null && events.size() > 0)
			{
				...
				int marriageYear = events.get(0).getYear() + this.MARRAGE_YEAR;
